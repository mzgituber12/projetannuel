...
async function signinUser(email, password) {
    ...
    if (!data.token || data.token == "") {
        window.location.href = "connexion.php?message=" + encodeURIComponent(data.message);
    } else {
        localStorage.setItem('token', data.token);
        window.location.href = "index.php?message=" + encodeURIComponent(data.message);
    }
}
signinUser(<?php echo json_encode($_POST['email']); ?>, <?php echo json_encode($_POST['password']); ?>);
</script>
