...
func Creer_evenement(database *sql.DB) http.HandlerFunc {
	return func(response http.ResponseWriter, request *http.Request) {
		...
		updatestatement, updateerr := database.Prepare("INSERT INTO evenement (nom, date, description, tarif) VALUES (?, ?, ?, ?)")
		if updateerr != nil {
			http.Error(response, "Erreur lors de la préparation de la requête de creation", http.StatusInternalServerError)
			return
		}
		_, updateexecerr := updatestatement.Exec(event.Nom, event.Date, event.Description, event.Tarif)
		if updateexecerr != nil {
			http.Error(response, "Erreur lors de la creation de l'événement", http.StatusInternalServerError)
			return
		}
