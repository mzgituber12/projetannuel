...
func Modifier_service(database *sql.DB) http.HandlerFunc {
	return func(response http.ResponseWriter, request *http.Request) {
		...
		verifemail, err := database.Prepare("SELECT nom FROM service WHERE nom = ? AND id_service != ?")
		if err != nil {
			http.Error(response, "Erreur lors de la vérification de l'existence du nom", http.StatusInternalServerError)
			return
		}
		var existingName string
		err = verifemail.QueryRow(serv.Nom, id).Scan(&existingName)
		if err == nil {
			response.Header().Set("Content-Type", "application/json")
...
func Creer_service(database *sql.DB) http.HandlerFunc {
	return func(response http.ResponseWriter, request *http.Request) {
		...
		updatestatement, updateerr := database.Prepare("INSERT INTO service (nom, description, tarif) VALUES (?, ?, ?)")
		if updateerr != nil {
			http.Error(response, "Erreur lors de la préparation de la requête de creation", http.StatusInternalServerError)
			return
		}
		_, updateexecerr := updatestatement.Exec(service.Nom, service.Description, service.Tarif)
		if updateexecerr != nil {
