...
func Modifier_abonnement(database *sql.DB) http.HandlerFunc {
	return func(response http.ResponseWriter, request *http.Request) {
		...
		selectstatement, selecterr := database.Prepare("SELECT categorie, type, prix_mois, prix_an, contenue1, contenue2, contenue3, contenue4, nb_avantage FROM abonnement WHERE id_abonnement = ?")
		...
		err := selectstatement.QueryRow(id).Scan(&data.Categorie, &data.Type, &data.PrixMois, &data.PrixAn, &data.Contenue1, &data.Contenue2, &data.Contenue3, &data.Contenue4, &data.Nb_avantage)
...
func Update_abonnement(database *sql.DB) http.HandlerFunc {
	return func(response http.ResponseWriter, request *http.Request) {
		...
		selectstm, err := database.Prepare("UPDATE abonnement set categorie = ?, type = ?, prix_mois = ?, prix_an = ?, contenue1 = ?, contenue2 = ?, contenue3 = ?, contenue4 = ?, nb_avantage = ? WHERE id_abonnement = ?")
