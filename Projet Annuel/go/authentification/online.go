...
func Fin_tuto(database *sql.DB) http.HandlerFunc {
	return func(response http.ResponseWriter, request *http.Request) {
		...
		if err != nil {
			http.Error(response, "Erreur lors de la mise a jour"+err.Error(), http.StatusNotFound)
			return
		}
...
func Reco_tuto(database *sql.DB) http.HandlerFunc {
	return func(response http.ResponseWriter, request *http.Request) {
		...
		slect, err := database.Prepare("UPDATE utilisateur SET tutoriel = 1 WHERE token = ?")
		if err != nil {
			http.Error(response, "Utilisateur non trouvé", http.StatusNotFound)
			return
		}
